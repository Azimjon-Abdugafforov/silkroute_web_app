...
const selectService = (value: IServices) => {
  services.value.forEach((item) => {
    if (item.name === value.name) {
      ...
      if (!item.selected) {
        const index = orderData.value.services.findIndex((s) => s.name === item.name);
        if (index !== -1) {
          orderData.value.services.splice(index, 1);
        }
      } else {
        const exists = orderData.value.services.some((s) => s.name === item.name);
        if (!exists) {
          orderData.value.services.push(item.name);
        }
      }
    }
  });
...
const rooms = ref([{ value: 1 }, { value: 2 }, { value: 3 }, { value: 4 }]);
</script>
